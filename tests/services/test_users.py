...
class TestUserService:
    ...
    def test_validate_user_email_success(self, service):
        valid_emails = [
            "user@example.com",
    ...
    def test_validate_user_email_invalid_format(self, service):
        invalid_emails = [
            "user@.com",
            "@example.com",
            "user@example",
    ...
    def test_validate_user_email_too_long(self, service):
        local_part = "a" * (user_settings.max_email_local_part + 1)
        email = f"{local_part}@example.com"
        with pytest.raises(user_errors.EmailFormatError):
            service.validate_user_email(email)

    def test_validate_user_email_double_dot(self, service):
        with pytest.raises(user_errors.EmailFormatError):
            service.validate_user_email("user..name@example.com")

    def test_validate_user_name_success(self, service):
        valid_names = [
            "John Doe",
            "Анна Петрова",
            "John_Doe",
    ...
    def test_validate_user_name_too_short(self, service):
        with pytest.raises(user_errors.NameFormatError):
            service.validate_user_name("")

    def test_validate_user_name_too_long(self, service):
        long_name = "a" * (user_settings.max_user_name_length + 1)
        with pytest.raises(user_errors.NameFormatError):
            service.validate_user_name(long_name)

    def test_validate_user_name_starts_with_digit(self, service):
        with pytest.raises(user_errors.NameFormatError):
            service.validate_user_name("123John")

    def test_validate_user_name_invalid_chars(self, service):
        with pytest.raises(user_errors.NameFormatError):
            service.validate_user_name("John@Doe")

    def test_validate_password_length_success(self, service):
        valid_passwords = [
            "Pass123!@#",
            "SecurePwd123$",
            "Aa1!Bb2@",
    ...
    def test_validate_password_length_too_short(self, service):
        with pytest.raises(user_errors.WeakPasswordError):
            service.validate_password_length("A1!")

    def test_validate_password_length_no_digit(self, service):
        with pytest.raises(user_errors.WeakPasswordError):
            service.validate_password_length("Password!@#")

    def test_validate_password_length_no_letter(self, service):
        with pytest.raises(user_errors.WeakPasswordError):
            service.validate_password_length("123456!@#")

    def test_validate_password_length_no_special(self, service):
        with pytest.raises(user_errors.WeakPasswordError):
    ...
    def test_create_user_success(self, mock_warning, mock_info, mock_hash, service, mock_db):
    ...
    @patch.object(UserService.logger, "warning")
    def test_create_user_already_exists(self, mock_logger, service, mock_db):
    ...
    def test_get_access_token(self, mock_jwt_encode, service, mock_user):
    ...
    @patch.object(UserService.logger, "warning")
    def test_get_user_not_found(self, mock_logger, service, mock_db):
        mock_db.query.return_value.filter.return_value.first.return_value = None
    ...
    @patch.object(UserService.logger, "warning")
    def test_verify_password_success(self, mock_warning, mock_verify, service, mock_user):
    ...
    @patch("src.services.users.verify_password")
    @patch.object(UserService.logger, "warning")
    def test_verify_password_invalid(self, mock_warning, mock_verify, service, mock_user):
    ...
    @patch.object(UserService.logger, "info")
    def test_change_password_success(self, mock_logger, mock_hash, service, mock_user, mock_db):
        service.change_password(mock_user, "new_password")

        assert mock_user.password_hash == "new_hashed_pwd"
        mock_db.flush.assert_called_once()  # Проверяем реальный mock_db.flush
        mock_logger.assert_called_once()
        mock_hash.assert_called_once_with("new_password")

    def test_get_instructor_courses(self, service, mock_user, mock_db, mock_course):
    ...
    @patch.object(UserService.logger, "info")
    @patch.object(UserService.logger, "warning")
    def test_delete_user_success(self, mock_warning, mock_info, service, mock_user, mock_db):
    ...
    @patch.object(UserService.logger, "warning")
    def test_delete_user_last_admin(self, mock_logger, service, mock_user, mock_db):
    ...
    @patch.object(UserService.logger, "info")
    def test_give_admin_permissions(self, mock_logger, service, mock_user, mock_db):
        service.give_admin_permissions(mock_user)
        assert mock_user.isadmin is True
        mock_db.flush.assert_called_once()
        mock_logger.assert_called_once()

    def test_get_all_users(self, service, mock_db):
    ...
    def test_get_all_users_empty(self, service, mock_db):
        mock_db.query.return_value.all.return_value = []

        result = service.get_all_users()
        assert result == []

    def test_get_admins(self, service, mock_db):
    ...
    def test_get_admins_empty(self, service, mock_db):
